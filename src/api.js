...
export const api = {
  ...
  removeAdminEmployee: (phone) => request(`/api/admin/employees/${encodeURIComponent(phone)}`, { method: 'DELETE' }),
  ...
  setCustomerDisabled: (phone, disabled) => request(`/api/customers/${encodeURIComponent(phone)}/disabled`, { method: 'PUT', body: JSON.stringify({ disabled }) }),
  deleteCustomer: (phone) => request(`/api/customers/${encodeURIComponent(phone)}`, { method: 'DELETE' }),
  ...
  listPayments: (phone) => request(`/api/payments${phone ? `?phone=${encodeURIComponent(phone)}` : ''}`),
  createPayment: (data) => request('/api/payments', { method: 'POST', body: JSON.stringify(data) }),
  getPaymentProof: (id, phone) => request(`/api/payments/${encodeURIComponent(id)}/proof?phone=${encodeURIComponent(phone || '')}`),
  approvePayment: (id) => request(`/api/payments/${encodeURIComponent(id)}/approve`, { method: 'POST', body: JSON.stringify({}) }),
  rejectPayment: (id, note) => request(`/api/payments/${encodeURIComponent(id)}/reject`, { method: 'POST', body: JSON.stringify({ note }) }),
